"""Generate the public technical report from persisted experiment evidence."""
...
def load(name):
    return json.loads((A / f"{name}.json").read_text())


m = load("metadata")
metrics = load("metrics")
temporal = load("temporal_metrics")
cv = pd.read_csv(A / "cv_summary.csv")
rows = [
...
for name, v in metrics.items():
    rows.append(
        f"| {name} | {v['fail_recall']:.3f} | {v['fail_precision']:.3f} | {v['f1']:.3f} | {v['pr_auc']:.3f} | {v['tp']} / {v['fn']} / {v['fp']} |"
    )
text = (
